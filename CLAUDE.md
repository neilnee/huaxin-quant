...
```
instructions/       - 各模型的执行指令（markdown，LLM 可直接执行）
  archive/          - 往期发布版归档（仅保留历史版本，不直接执行）
...
- **指令卡保持精简**：指令文件只保留 LLM 执行所需的内容（流程、规则、约束）。公式速查、报告模板、字段定义等参考内容放入配对文件 `-ref-dev.md`，执行时不加载，按需查阅。版本历史、踩坑记录、开发复盘等写入 `dev_logs/`，不在指令文件中展开
- **开发与发布分离**：日常迭代只动 `-dev.md` 和 `-ref-dev.md` 开发版，不动 `-vX.Y.md` 发布版。开发版稳定后，同时复制指令卡和参考卡到发布版并升级版本号，发布版只读
- **版本号约定**：默认小版本号累加（v1.5→v1.6→v1.7），仅用户明确说"大版本""升级到 v2.0"等时才升主版本号。日常迭代不问版本号，直接小版本 +1
- **指令版本归档**：`instructions/` 外层只保留 `-dev.md` + `-ref-dev.md` 开发版 + 最新 `-vX.Y.md` + `-ref-vX.Y.md` 发布版。发布新版时，将旧发布版移入 `instructions/archive/`，保持外层简洁
- **发版收尾**：每次发版完成后，更新 `TODO.md`——勾掉本次发版已完成的待办项，将已完成的从"已完成"区清理（保留近一周的供复盘），新增发版过程中识别的新待办
