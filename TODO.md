...
- [ ] **只读数据适配**：消费当日 `bloom/review_input/bloom_input_<YYMMDD>.json`、Bloom 状态快照和模型二必要字段，生成 `dashboard/data/vcp_context_<YYMMDD>.js`；不改变 Quant 或 Bloom 的既有 JSON、CSV、Markdown 与状态机。
...
- [ ] **VCP 标的列表**：以代码、名称、结构阶段、Bloom 状态、当前/潜在买点、结构质量、风险、距 Pivot/MA20 位置为核心列；列表只承担筛选和定位。
- [ ] **标的详情**：展示收缩轮次、收缩幅度与量能、Pivot/支撑/失效价、MA20/MA60、触发条件、风险标记、Bloom 生命周期与 LLM 个股解读；明确区分模型二事实与 Bloom 生命周期判断。

**后续增强与验证：**

- [ ] **板块环境关联（二期）**：详情页关联所属申万二级与概念的市场环境数据，仅作上下文展示，不改写 VCP 结构或 Bloom 状态。
