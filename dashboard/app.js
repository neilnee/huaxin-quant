...
function vcpStatus(value) { const labels={TRIGGERED:"已触发",MATURE:"成熟",FORMING:"形成中",EARLY:"早期观察",RISK_BLOCKED:"风险阻断"}; return `<span class="status-pill status-${String(value||"").toLowerCase().replaceAll("_","-")}">${labels[value]||esc(value||"—")}</span>`; }
...
function signalRiskLabel(value) {
  return {INTRADAY_CLOSE_DIVERGENCE:"盘中振幅显著大于收盘波动"}[value] || value;
}
...
function renderDetail(row) {
  ...
  $("sector-detail").innerHTML=`<h3>${esc(row.block_name)}</h3>${sectorStateTag(row.sector_state)}<div class="detail-grid detail-primary">${metric("20日排名",`#${row.rank_20}`)}${metric("5日排名",`#${row.rank_5}`)}${metric("20日相对强度",pct(row.relative_strength_20),cls(row.relative_strength_20))}${metric("5日相对强度",pct(row.relative_strength_5),cls(row.relative_strength_5))}</div><p class="detail-heading">收益与参与度</p><div class="detail-grid">${metric("20日中位收益",pct(row.return_20),cls(row.return_20))}${metric("成员数",row.member_count)}${metric("上涨占比",pct(row.up_breadth))}${metric("量能活跃",`${row.volume_activity.toFixed(2)}x`)}</div><p class="detail-heading">趋势扩散</p><div class="detail-grid">${metric("站上 MA20",pct(row.above_ma20_ratio))}${metric("站上 MA60",pct(row.above_ma60_ratio))}${metric("60日新高",pct(row.new_high_ratio))}${metric("强势股密度",pct(row.strong_stock_density))}</div>${leaderRows?`<section class="sector-leaders"><p class="detail-heading">板块强势股 <span>Top 3 · 量价强度</span></p>${leaderRows}</section>`:""}<p class="detail-meta">阶段数据 ${history.length}/20 日｜${row.history_basis==="point_in_time"?"每日快照（点时）":"当前成分快照回填（非严格点时）"}</p><p class="detail-heading">近期阶段轨迹 <span>最新 10 日</span></p><table class="detail-history"><thead><tr><th>日期</th><th>20日排名</th><th>5日排名</th><th>20日相对</th><th>5日相对</th></tr></thead><tbody>${trajectory||"<tr><td colspan='5' class='muted'>历史积累中</td></tr>"}</tbody></table>`;
  const panel=$("sector-detail"),stateTag=panel.querySelector(".sector-state");
  if(stateTag) panel.querySelector("h3")?.append(" ",stateTag);
