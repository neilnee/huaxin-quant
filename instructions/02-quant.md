...
随后 1-15 个交易日内缩量回踩；按快速、标准、迟到窗口分层评价
回踩不有效跌破突破位
重新站回突破位或 MA10
```
...
| `dashboard/data/<YYYYMM>/signals_context_<YYMMDD>.js` | 只读信号发现页面数据包，展示当日触发与次日计划，不改变模型二判定；发布器 `--all` 必须覆盖已存在的完整模型二历史日期 |
...
BREAKOUT_BUY
- VCP 枢轴突破参与点。
- 前提形态：当前存在有效 VCP 结构；两轮以上至少为 `VCP_FORMING`，单轮仅允许已确认且达到更严格质量门槛的 `VCP_EARLY`。
- 硬条件：按有效收缩轮次复用“突破后生命周期”中的同一分级门槛；成交量必须同时达到对应的 `vol_ma20` 与 `vol_ma5` 要求，突破日须为强收盘且无明显长上影/放量滞涨，并且无趋势硬风险。
- 单轮结构：收盘至少站上 Pivot 2%，量能至少为 MA20 的 1.30 倍且为 MA5 的 1.20 倍，结构量能须为 `drying/decreasing`，买点总分至少 65。
- 两轮及以上：收盘至少站上 Pivot 1%，量能不得低于 MA20 和 MA5，买点总分至少 55。
...
原 VCP 的正式 `price_breakout` 必须在最后一轮收缩后同时满足按有效收缩轮次分级的价格、量能和 K 线质量门槛。普通价格越过 Pivot 但未达到对应质量门槛时，继续保持原 `PRE_BREAKOUT` 结构，不消费 contraction group，也不启动突破后失败或过期生命周期。

- **一轮有效收缩**：该轮必须为 `CONFIRMED`，收盘站上 `structure_pivot × 1.02`，成交量同时不低于 `vol_ma20 × 1.30` 与 `vol_ma5 × 1.20`，结构量能为 `drying/decreasing`，且突破日为强收盘、无明显长上影。高质量单轮允许形成正式突破，但 `BREAKOUT_BUY` 总分门槛提高到 65。
- **两轮及以上有效收缩**：最右端实时候选可以为 `PROVISIONAL`，收盘站上 `structure_pivot × 1.01`，成交量必须同时不低于 `vol_ma20` 与 `vol_ma5`，且突破日收盘质量合格、无明显长上影。`BREAKOUT_BUY` 总分门槛保持 55。

突破质量判定必须由结构生命周期、突破消费边界和 `BREAKOUT_BUY` 复用同一实现。不得出现生命周期已认定突破、买点层却因同日量价质量不足而否定突破的双重口径。正式突破后的失败线、最大回撤、回踩窗口和不可逆规则保持不变。
...
候选组还必须遵守突破消耗边界：若候选组的前缀已经在后一轮收缩开始前按上述分级门槛确认正式 `price_breakout`，该前缀已被突破消耗，候选组不得再把突破后的收缩拼回旧 VCP。一轮高质量收缩也允许形成消费边界；未达到质量门槛的普通 Pivot 越界不得切断前后收缩。

“质量合格的价格突破边界”不自动等于“有效 VCP 突破”。消费边界首先必须通过同一分轮次质量门槛；即使旧组趋势资格不足，质量合格的边界仍可切断前后收缩，避免后验行情把下降趋势中的旧波动拼入新 VCP。只有再使用突破日前最后一个交易日的数据重跑完整 VCP 判定，满足 `has_structure=true`、当前有效组与产生 Pivot 的组一致，才允许进入有效 VCP 的突破后生命周期、附加分或专用回踩计划。两轮及以上要求突破前阶段至少为 `VCP_FORMING`；一轮只允许 `CONFIRMED` 的高质量 `VCP_EARLY`。不得使用突破日之后的数据反向改善前序结构资格。

模型二另外运行不授予买点权限的右侧重建发现旁路。该旁路的失败边界必须遵守同一资格：只有突破前已通过上述完整 VCP 判定的旧组，才有资格约束旁路的新结构选组；枚举候选组时产生的单轮收缩、趋势观察组、被拒绝组或其他未取得有效 VCP 突破资格的组合，不得误伤无关的新结构。正式交易主路径继续保留原有保守失败边界，避免规则升级直接恢复旧组或新增买点。

有效旧 VCP 的失败仍然不可逆，永久关闭该旧组、旧 Pivot 及跨越失败边界混用旧收缩的候选组。发现旁路可以独立识别失败后或无资格旧组之外的新收缩，但统一输出 `TREND_REBUILD / WAIT_REBUILD`；不得因为旁路发现新结构而恢复旧结构或新结构的 PULLBACK、BREAKOUT、RETEST 权限。只有正式交易主路径按原规则确认标准新 VCP 后，才恢复正常买点评价。

发现旁路不得参与任何历史评分锚点重建。突破前 VCP 资格、`setup_structure_score` 与冻结的 `structure_breakout_score` 必须关闭该旁路后按正式交易主路径重算，避免研究标签反向改写旧结构质量或影响突破后跟踪排序。

### 失败后的低量重建观察

有效旧 VCP 失败后，或不具备有效 VCP 资格但此前存在明确放量启动时，若右侧尚不足以形成标准 VCP、但已经出现低量稳定平台，模型二应输出研究级重建观察，不得只展示旧结构失败或普通风险排除：

```text
structure_stage = TREND_REBUILD
setup_signal = NONE
action_hint = WAIT_REBUILD
prior_breakout_context_tag = 放量启动后低量重建
```

观察来源必须二选一：有效旧 VCP 的失败生命周期；或最近回看窗口内至少上涨规定幅度、峰值量达到此前基准均量倍数的独立放量启动。若已经识别到右侧候选收缩，其量能必须为 `drying/decreasing`，`mixed/failed` 不得仅凭放宽后的候选组进入重建观察。若尚无候选收缩，则必须同时满足：启动或失败后至少经过配置要求的连续交易日；最近窗口收盘振幅受控、当前收盘相对启动后最低收盘已有最小企稳幅度；最近窗口均量相对启动峰值量显著下降；当前价相对启动后最高收盘的回撤未超过上限；启动后的最低收盘已经经过足够交易日确认。该状态只扩大研究可见性，不形成正式 contraction group，不计入结构分加分，不生成任何买点或买点计划。达到标准新 VCP 门槛后，自动退出该观察状态并按新结构正常评价。
...
默认必须先有 `VCP_FORMING`、`VCP_MATURE` 或 `VCP_TIGHT`。只有单轮已确认且通过上述更高突破质量门槛的 `VCP_EARLY`，才允许触发 `BREAKOUT_BUY`。
...
两轮及以上：当日成交量同时 >= vol_ma20 × 1.0 与 vol_ma5 × 1.0
单轮已确认：close > pivot × 1.02，且成交量同时 >= vol_ma20 × 1.30 与 vol_ma5 × 1.20
无明显长上影
无放量滞涨
...
```text
突破日当天不属于回踩；突破后 1-15 个交易日内按统一时间窗判断
...
时间窗口不设置额外的最少等待日数，只排除突破当天；最大允许天数为硬边界：

| `setup_timing` | 突破后交易日 | 处理 |
|---|---:|---|
| `FAST` | 1-2 日 | 允许触发，仅标记快速回踩，不限制 `setup_quality` |
| `STANDARD` | 3-10 日 | 标准回踩窗口，不增加时间质量上限 |
| `LATE` | 11-15 日 | 允许触发，但因旧突破确认衰减，`setup_quality` 最高为 C |
| `OUTSIDE` | 0 日或超过 15 日 | 不触发 `RETEST_BUY` |

`FAST` 不因确认时间短而降级：价格位置、缩量、重新确认、突破质量和突破前 VCP 质量已经共同评价买点，满足高质量条件时可以达到 A。`LATE` 的时间上限与结构量能上限共同生效，取更弱者，例如 `LATE + mixed` 最高 C。只要仍处于允许窗口且每日量价条件成立，`RETEST_BUY` 可以连续出现；这是当日机会提示，不读取持仓、不推断加仓，也不等同于实际交易动作。
