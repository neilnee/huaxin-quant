...
> 当前执行指令见 [03-valuation.md](03-valuation.md)。本文件保存旧引擎公式、接口和模板，**只在维护对应引擎或审计历史运行包时按需加载**。其中 A–F、可比 PE、Layer 加值和旧模板不覆盖 V4 机构共识协议；不得因为此处列有公式而补造新报告目标估值。

文档边界核对：2026-09-08。历史研究协议见 [03-valuation-legacy-ref.md](03-valuation-legacy-ref.md)。
...
1. 结论摘要
2. 公司整体一致性预期估值表（双年三情景：净利润、目标PE、目标价、估值）
3. 业务支柱拆分（只解释业务与利润来源，不分配估值）
4. 逐机构利润预测与关键假设
5. 机构一致预期明细表
6. 关键分歧
7. 后续验证节点
...
不展示PE二次构建、可比估值、独立事项重复估值、核心假设汇总、催化剂和已验证事实。
