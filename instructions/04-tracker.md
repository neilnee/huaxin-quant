...
```text
共享市场数据更新 → Pool → Quant → Bloom → Signal Plan → 信号财务提示
→ 当天完整资金观测 → 市场/回测/VCP 页面发布 → 信号股资金补查与信号页面发布
→ AI研读数据包 → 当日产物完整性核验 → 打开面板 → 可选自选重建
```

完整资金观测必须生成同日独立资金页；信号股资金补查必须覆盖当日触发股与 Signal Plan 股票。两者使用独立请求预算，任一任务不得被另一任务替代。每日默认工作流在页面发布完成后额外执行自选重建：

AI研读数据包必须在市场、资金、VCP、信号四类 Dashboard 数据包全部发布后执行，调用
`scripts/daily_ai_report.py --date <YYMMDD>`；它是确定性最终汇总，不调用新的LLM。生成失败属于阻断错误，
且其月目录JSON必须进入当日产物完整性核验。
