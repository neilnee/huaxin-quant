...
字段解析优先使用妙想 `rawTable + nameMap`，同时保存最近成功返回的底层字段编码和字段名作为响应契约。字段编码变化时，只有新旧字段名都能通过受控别名表解析为同一标准指标才允许刷新契约；例如“成交额(合计)”与“成交额”、“(区间)主力净流入资金”与“主力净流入资金”属于同一受控语义。只有 ID 相同不能弥补字段名语义异常。金额统一转换为元，百分比保留数值百分比。无法识别的 `-`、空值或未披露值保存为 `null`。
