...
妙想 `rawTable` 的底层字段 ID 可因查询模板、单日/区间形态或接口版本发生变化，不能把 ID 本身作为唯一语义契约。契约校验必须同时保留字段 ID 和字段名：当新旧 ID 不同但两个字段名仍能由受控别名表唯一解析到同一标准指标时，允许接受并更新最近 ID；字段名无法映射、映射到其他指标或必需字段缺失时必须继续拒绝入库。
