...
板块排名主表仅展示 20 日排名、板块、当前状态、20 日相对强度、5 日相对强度，用于快速筛选，宽度应显著小于右侧详情，表头居中。当前状态使用颜色标签区分持续主线、新晋强化、高位分歧、轮动脉冲、观察中、弱势退潮和历史积累。右侧详情承担阶段解释：以紧凑四列网格展示排名、相对强度、20 日中位收益、成员数、上涨扩散、量能、MA20/MA60 覆盖、60 日新高、强势股密度；阶段轨迹默认仅展示最新 10 日，完整历史由下方排名矩阵提供。矩阵应完整展示最近 20 个交易日，不设置纵向滚动截断；矩阵中的任意板块名称可点击，点击后该板块在所有日期的矩阵单元格以同色高亮，再次点击取消高亮。独立广度模块不展示；全 A 广度在市场状态详情中呈现，板块广度在板块详情中呈现。
