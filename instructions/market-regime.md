...
当前市场状态标题右侧提供灰色斜体下划线“详情”入口；点击后在整个市场状态卡下方展开/收起独立的“状态判定说明”模块。说明区先展示当前状态的实际值与已命中条件，再列出所有市场状态的触发规则、确认天数和含义，并说明趋势、波动、广度、轮动四项分数的计算口径。默认收起；说明仅展示脚本的既有规则和配置，不参与状态计算。
...
板块排名主表仅展示 20 日排名、板块、当前状态、20 日相对强度、5 日相对强度，用于快速筛选，宽度应显著小于右侧详情，表头居中。当前状态使用颜色标签区分持续主线、新晋强化、高位分歧、轮动脉冲、观察中、弱势退潮和历史积累。右侧详情承担阶段解释：以紧凑四列网格展示排名、相对强度、20 日中位收益、成员数、上涨扩散、量能、MA20/MA60 覆盖、60 日新高、强势股密度；阶段轨迹默认仅展示最新 10 日，完整历史由下方排名矩阵提供。5 日排名、20 日中位收益、广度与量能不在主表重复展示。
