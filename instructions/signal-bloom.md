...
个股资金验证只作为旁路证据，不得改写模型二买点、评分、等级或仓位。信号发布器按股票代码去重后复用 `capital_data.sqlite`，每批最多5只；展示主力资金状态、最新净流入率、近3日流入天数，以及融资资金状态、近5日融资余额变化和最新融资净买入额。数据包保留主力资金和融资资金的实际日期供审计，页面不单独展示。已查询但未返回融资字段时保持“数据缺失”，不得写成零。
